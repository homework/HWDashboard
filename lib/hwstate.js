(function() {
  ...
  HWState = (function() {
    ...
    HWState.prototype.getState = function() {
      return this.state;
    };
    HWState.prototype.updateState = function() {
      return this.updateDeviceNames(__bind(function() {
        console.log("devices");
        return setTimeout(__bind(function() {
          return this.updateUsers(__bind(function() {
            console.log("users");
            console.log("TT " + this.state.devices.get("1.1.1.7").has("user"));
            return setTimeout(__bind(function() {
              return this.updateAllowances(__bind(function() {
                console.log("allowances");
                console.log(this.state.devices.get("1.1.1.7").has("user"));
                return this.emit('update', this.state);
              }, this));
            }, this), 2000);
          }, this));
        }, this), 2000);
      ...
      this.hwdb.once('message', __bind(function(device_rows) {
        ...
        if (device_rows[0].status === "Success" && device_rows[0].rows > 0) {
          ...
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            ...
            if (this.state.devices[device_row.ip] != null) {
              this.state.devices[device_row.ip].set({
                name: device_row.name
              });
            } else {
              this.state.devices.add(new models.Allowance({
                id: device_row.ip,
                name: device_row.name
              }));
            }
    ...
    HWState.prototype.updateUsers = function(callback) {
      this.hwdb.once('message', __bind(function(user_rows) {
        ...
        if (user_rows[0].status === "Success" && user_rows[0].rows > 0) {
          ...
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            user_row = _ref[_i];
            if (this.state.devices.get(user_row.ip) != null) {
              console.log("Device exists");
              this.state.devices.get(user_row.ip).set({
                user: user_row.name
              });
              console.log(user_row.ip, this.state.devices.get(user_row.ip).has("user"));
            } else {
      ...
      this.hwdb.once('message', __bind(function(allowance_rows) {
        ...
        if (allowance_rows[0].status === "Success" && allowance_rows[0].rows > 0) {
          ...
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            ...
            if (this.state.devices.get(allowance_row.ip) != null) {
              console.log("Setting");
              this.state.devices.get(allowance_row.ip).set({
                allowance: parseInt(allowance_row.allowance)
              });
            } else {
              console.log("Adding");
              this.state.devices.add(new models.Allowance({
