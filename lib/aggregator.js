(function() {
  ...
  Aggregator = (function() {
    ...
    Aggregator.prototype.initialize = function() {
      ...
      cron('0 0 * * * *', function() {
        var date_now, end_date, start_date;
        date_now = new Date();
        ...
        return hwdb_aggregator.aggregateHour(start_date, __bind(function(result) {
          var bytes, date_string, ip, _results;
          console.log(result);
          if (result !== "empty") {
            _results = [];
            for (ip in result) {
              ...
              _results.push(mysql.query("INSERT INTO bandwidth_hours (date,hour,ip,bytes) VALUES (?,?,?,?)", [date_string, start_date.getUTCHours(), ip, bytes], function(e) {
