(function() {
  var Defragger, EventEmitter, HWDashboardLogger, JSRPC, defrag, hwdbparser, log, pktr;
  ...
  JSRPC = (function() {
    ...
    JSRPC.prototype.setupCommandListener = function() {
      return pktr.on("command", __bind(function(sub_port, seq_no, command, data, frag_count, frag_no) {
        ...
        if (frag_count === frag_no && frag_count !== 1) {
          if (defrag.getTotalLength() === 0) {
            return pktr.sendCommand(Command.RACK, "", sub_port, seq_no);
          } else {
            ...
            if (sub_port === outboundSubPort) {
              if (seq_no === outboundSeqNo || seq_no === outboundSeqNo + 1) {
                ...
                if (state === RPCState.AWAITING_RESPONSE) {
                  ...
                  if (seq_no === outboundSeqNo && defrag.getTotalLength()) {
                    if ((defrag.push(frag_count, data.slice(4))) === frag_count) {
              ...
              if (seq_no === inboundSeqNo || seq_no === inboundSeqNo + 1) {
                ...
                if (state === RPCState.IDLE) {
                  ...
                  if (seq_no === inboundSeqNo && defrag.getTotalLength()) {
    ...
    JSRPC.prototype.connect = function(address, port) {
      ...
      return setTimeout(function() {
        return pktr.sendCommand(Command.CONNECT, "HWDB\0", outboundSubPort, outboundSeqNo);
      }, 2000);
    };
