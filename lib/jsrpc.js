(function() {
  ...
  JSRPC = (function() {
    ...
    function JSRPC(address, port) {
      this.pktr = new Packeteer(address, port);
      ...
      return this.pktr.on("command", __bind(function(sub_port, seq_no, command, data, frag_count, frag_no) {
        ...
        if (frag_count === frag_no && frag_count !== 1) {
          if (this.defrag.getTotalLength() === 0) {
            ...
            this.setState(RPCState.AWAITING_RESPONSE, 1);
            this.log.debug("Final fragment: " + frag_count + "/" + frag_no);
              ...
              if (seq_no === this.outboundSeqNo || seq_no === this.outboundSeqNo + 1) {
                ...
                if (this.state === RPCState.AWAITING_RESPONSE) {
                  if (seq_no === this.outboundSeqNo && frag_count === 1) {
                    ...
                    return this.pktr.sendCommand(Command.FACK, "", sub_port, this.outboundSeqNo, frag_count, frag_no);
                  }
            ...
            if (seq_no === this.outboundSeqNo || seq_no === this.outboundSeqNo + 1) {
              ...
              switch (command) {
                ...
                case Command.RESPONSE:
                  if ((this.state === RPCState.AWAITING_RESPONSE) || (this.state === RPCState.QUERY_SENT) && (seq_no === this.outboundSeqNo)) {
                    this.log.debug("RESPONSE received: " + data);
                    this.setState(RPCState.IDLE, 1);
                    this.emit('message', hwdbparser.parseQueryOrResponse(data));
                    return this.pktr.sendCommand(Command.RACK, "", this.outboundSubPort, this.outboundSeqNo);
                  }
                  break;
                case Command.SACK:
                ...
                case Command.DACK:
                  if (this.state === RPCState.DISCONNECT_SENT) {
                    ...
                    this.setState(RPCState.TIMEDOUT, 1);
                    this.pktr.close();
            ...
            if (seq_no === this.inboundSeqNo || seq_no === this.inboundSeqNo + 1) {
              switch (command) {
                ...
                case Command.QUERY:
                  ...
                  this.setState(RPCState.RESPONSE_SENT, 0);
                  return this.pktr.sendCommand(Command.RESPONSE, "OK\0", this.inboundSubPort, this.inboundSeqNo);
                case Command.RACK:
    ...
    JSRPC.prototype.getConnected = function() {
    ...
    JSRPC.prototype.query = function(query) {
      ...
      if (this.connected) {
        ...
        if (this.outboundSeqNo >= 4294967294) {
          ...
          this.setState(RPCState.QUERY_SENT);
          return this.pktr.sendCommand(Command.QUERY, query, this.outboundSubPort, ++this.outboundSeqNo);
        }
      } else {
    ...
    JSRPC.prototype.disconnect = function() {
      ...
      this.setState(RPCState.DISCONNECT_SENT);
