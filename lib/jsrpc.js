(function() {
  var Defragger, EventEmitter, JSRPC, hwdbparser, pktr;
  ...
  JSRPC = (function() {
    ...
    JSRPC.prototype.setupCommandListener = function() {
      return pktr.on("command", __bind(function(sub_port, seq_no, command, data, frag_count, frag_no) {
        var fragment_length, result, total_length;
        ...
        if (command === Command.FRAGMENT) {
          if (sub_port === outboundSubPort) {
            if (seq_no < outboundSeqNo) {
              return console.log("Received old/repeat sequence number from responder during fragment");
            } else if (seq_no > (outboundSeqNo + 1)) {
              return console.log("Received sequence number too far ahead from responder during fragment");
            } else {
            ...
            if (seq_no < inboundSeqNo) {
              ...
              if (state === RPCState.RESPONSE_SENT) {
                if (seq_no === (inboundSeqNo + 1) && frag_no === 1) {
