(function() {
  ...
  DashORM = (function() {
      ...
      switch (model) {
        case "allowances":
          ...
          month_totals.on('row', __bind(function(mysql_row) {
            var current_household_usage, device;
    ...
    DashORM.prototype.liveUpdate = function(package) {
      var device, device_state, hh_usage, item, item_date, item_str, month_model, user_model, user_total_allowance, user_total_usage, _i, _len;
        ...
        if (month_model !== void 0) {
          ...
          if (device_state.has("user")) {
            ...
            if (user_model) {
              ...
              month_model.devices.each(__bind(function(mm_device) {
                if (this.state.devices[mm_device.id].get("user") === this.state.devices[item.ipaddr].get("user")) {
                  console.log("Matched user");
                  user_total_usage += mm_device.get("usage");
                  return user_total_allowance += mm_device.get("allowance");
                }
              }, this));
              month_model.users.add(new models.Allowance({
                id: this.state.devices[item.ipaddr].get("user"),
          ...
          if (device) {
            ...
            month_model.devices.add(new models.Allowance({
              id: this.state.devices[item.ipaddr],
              usage: parseInt(item.bytes),
              allowance: parseInt(this.state.devices[item.ipaddr].get("allowance"))
            }));
          }
        }
      }
