(function() {
  var EventEmitter, HWDBAggregator, JSRPC, hwdb_historical, hwdb_live, __;
  ...
  HWDBAggregator = (function() {
    ...
    HWDBAggregator.prototype.initialize = function() {
      ...
      return console.log("Double connect");
    };
    ...
    HWDBAggregator.prototype.aggregateQuery = function(start, end, callback) {
      ...
      hwdb_historical.once('message', __bind(function(rows) {
        ...
        hwdb_live.once('message', __bind(function(rows) {
          var hour_result;
          if (rows[0].status === "Success") {
            if (rows[0].rows > 0) {
        ...
        return hwdb_live.query(query);
      }, this));
      return hwdb_historical.query(query);
    };
    HWDBAggregator.prototype.destroy = function() {
      hwdb_live.disconnect();
      hwdb_live.on('disconnected', function() {
        return console.log("Live DC");
      });
      hwdb_historical.disconnect();
      return hwdb_historical.on('disconnected', function() {
